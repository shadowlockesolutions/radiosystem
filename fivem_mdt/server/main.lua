...
local function fetchOpenCalls()
    local rows = MySQL.query.await('SELECT id, call_number, call_type, details, caller_name, location_json, priority, created_at FROM mdt_dispatch_calls WHERE status = ? ORDER BY created_at DESC LIMIT 200', { 'open' })
    local calls = {}
    for _, row in ipairs(rows) do
        local location = row.location_json and json.decode(row.location_json) or nil
        calls[#calls + 1] = {
            dbId = row.id,
            id = row.call_number,
            type = row.call_type,
...
local function registerCall(callType, payload)
    local locationJson = payload.location and json.encode(payload.location) or nil
    MySQL.insert.await([[
        INSERT INTO mdt_dispatch_calls (call_number, call_type, details, caller_name, location_json, source_type, priority, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'open')
    ]], {
        payload.id,
        payload.type or callType,
        payload.details,
        payload.callerName,
        locationJson,
        payload.sourceType or 'system',
        tonumber(payload.priority) or 2
    })
...
RegisterNetEvent('rs_mdt:server:resolveCall', function(callId)
    ...
    MySQL.update.await('UPDATE mdt_dispatch_calls SET status = ?, resolved_by = ?, resolved_at = NOW() WHERE call_number = ? AND status = ?', {
        'resolved',

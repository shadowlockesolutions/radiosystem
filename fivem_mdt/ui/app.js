...
const renderActiveCalls = (rows = []) => renderCards(activeCallList, rows.map((c) => `<article class="card"><strong>${c.id} - ${c.type}</strong><small>${c.details || ''}</small><button onclick="resolveCall('${c.id}')">Resolve</button></article>`));
